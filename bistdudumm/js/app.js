...
import { Ai, AiNotConfiguredError, AiError } from './ai.js';
...
/* ============================================================================
   NAVIGATION
   ========================================================================== */
const SCREENS = ['splash', 'home', 'prep', 'round', 'result', 'board', 'legal'];
...
function renderHome() {
  ...
  $('#homeSub').textContent =
    `${Store.settings.roundSeconds} Sekunden pro Runde · null Ausreden`;
...
/* ============================================================================
   ...
   ========================================================================== */
function openLegal(tab = 'impressum') {
  G.legalTab = tab;
  show('legal');
  renderLegal();
}
...
function renderLegal() {
  const isImp = G.legalTab !== 'datenschutz';
  $('#legalTitle').textContent = isImp ? 'Impressum' : 'Datenschutz';
  $('#legalBody').innerHTML = isImp ? impressumHtml() : datenschutzHtml();
  $$('#legalTabs button').forEach(b =>
    b.classList.toggle('is-on', (b.dataset.legal === 'datenschutz') === !isImp));
  const sc = $('#screen-legal .scroll');
  if (sc) sc.scrollTop = 0;
}
...
function openSettings() {
  ...
  openSheet('Einstellungen', `
    ...
    <button class="btn btn-ghost btn-sm" id="btnLegal" style="margin-top:16px">
      📄 Impressum & Datenschutz
    </button>
    <button class="btn btn-ghost btn-sm" id="btnClearPools" style="margin-top:10px">
  ...
  `, body => {
    ...
    on($('#btnLegal', body), 'click', () => { closeSheet(); openLegal('impressum'); });
...
function openCategoryEditor(existing = null) {
  ...
  openSheet(existing ? 'Kategorie bearbeiten' : 'Eigene Kategorie', `
    ...
    <div class="card ai-card" style="margin-bottom:16px;border-style:dashed">
      <div class="eyebrow" style="margin-bottom:8px">✨ Mit KI füllen</div>
      <input class="input" id="aiTopic" placeholder="${esc(T.AI_PLACEHOLDER)}" enterkeyhint="go">

      <div class="field" style="margin:12px 0 10px">
        <label>Wie viele Begriffe?</label>
        <div class="seg" id="aiCount">
          ${CONFIG.ai.counts.map(n =>
            `<button data-n="${n}" class="${n === CONFIG.ai.defaultCount ? 'is-on' : ''}">${n}</button>`
          ).join('')}
        </div>
      </div>
      ...
      <p class="tiny" id="aiHint" style="margin-top:10px;line-height:1.5">
        ${Ai.available
          ? 'Thema eintippen, Rest macht die KI. Danach in Ruhe durchsehen.'
          : esc(T.AI_NOT_CONNECTED)}
      </p>
    </div>
  ...
  `, body => {
    ...
    let aiCount = CONFIG.ai.defaultCount;
    on($('#aiCount', body), 'click', e => {
      const b = e.target.closest('button[data-n]'); if (!b) return;
      aiCount = +b.dataset.n;
      $$('button', $('#aiCount', body)).forEach(x => x.classList.toggle('is-on', x === b));
      Sound.tap(); Haptics.light();
    });

    const runAi = async () => {
      const btn = $('#aiGo', body);
      const hint = $('#aiHint', body);
      const topic = $('#aiTopic', body).value.trim() || $('#ccName', body).value.trim();
      if (!topic) {
        toast('Erst ein Thema, dann Magie.'); Sound.error();
        $('#aiTopic', body).focus();
        return;
      }
      const before = parseWords($('#ccWords', body).value);

      btn.disabled = true;
      btn.classList.add('is-busy');
      btn.textContent = 'Denkt nach …';
      hint.textContent = `Erzeuge ${aiCount} Begriffe zu „${topic}“ …`;

      try {
        const words = await Ai.generate({ topic, count: aiCount, spicy, exclude: before });
        const ta = $('#ccWords', body);
        const merged = parseWords([...before, ...words].join('\n'));
        const added = merged.length - before.length;
        const dupes = words.length - added;
        ta.value = merged.join('\n');
        count();
        Sound.fanfare(); Haptics.success();
        hint.textContent = added
          ? `${added} neue Begriffe dazu${dupes ? `, ${dupes} waren schon drin` : ''}. Jetzt durchsehen und Unpassendes löschen.`
          : 'Alles, was zurückkam, war schon vorhanden. Thema etwas anders formulieren?';
        if (added) toast(`${added} Begriffe dazu.`);
      } catch (err) {
        Sound.error(); Haptics.fail();
        hint.textContent = err instanceof AiNotConfiguredError
          ? T.AI_NOT_CONNECTED
          : (err instanceof AiError ? err.message : `Hat nicht geklappt: ${err.message}`);
      } finally {
        btn.disabled = false;
    ...
    on($('#aiGo', body), 'click', runAi);
    on($('#aiTopic', body), 'keydown', e => { if (e.key === 'Enter') { e.preventDefault(); runAi(); } });
...
function wireEvents() {
  ...
  on($('#screen-home'), 'click', e => {
    const b = e.target.closest('[data-legal]'); if (!b) return;
    Sound.tap(); openLegal(b.dataset.legal);
  });
  on($('#btnLegalBack'), 'click', () => { Sound.back(); show('home'); });
  on($('#legalTabs'), 'click', e => {
    const b = e.target.closest('button[data-legal]'); if (!b) return;
    G.legalTab = b.dataset.legal; Sound.tap(); renderLegal();
  });
