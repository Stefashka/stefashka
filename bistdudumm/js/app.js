...
function updatePrepOrientation() {
  ...
  const isPhone = Math.min(vw, vh) < 600;      // Tablets/Desktop bleiben, wie sie sind
  const portrait = vh > vw;

  // Was sagt der Sensor? 90/270 = das Handy wird bereits quer gehalten.
  const sensed = (G.sensorState === 'granted' && motion.hasData) ? motion.screenAngle() : 0;
  const held = sensed === 90 || sensed === 270;

  const target = held ? sensed : (isPhone && portrait ? 90 : 0);
  G.prepHinting = !held && isPhone && portrait;
...
function startRound() {
  ...
  // Nulllage EXAKT jetzt einfrieren + Lesewinkel bestimmen.
  // Ohne Sensor übernehmen wir den Winkel der Anleitung – die hat ja gerade
  // erst gesagt „quer halten“, also muss der Begriff genauso stehen.
  const angle = sensorOn ? motion.freeze() : (G.prepAngle || 0);
