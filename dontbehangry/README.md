...
Server-Logik. Doppelklick auf `index.html` genügt zum Ansehen.

Die Seite liegt online unter `stefashka.github.io/dontbehangry/`, im Repo also
im Ordner `dontbehangry` neben der Startseite. Alle Verweise im Code sind
relativ zu `index.html` – der Ordner kann heißen wie er will und beliebig tief
liegen, solange er als Ganzes umzieht.
...
Zum schnellen Ansehen genügt ein Doppelklick auf `index.html`.

Näher am Ernstfall ist ein kleiner Server im Projektordner:

```bash
cd <Projektordner> && python3 -m http.server 8000
```

Dann `http://localhost:8000/` aufrufen. Das lohnt sich, weil ein Server sich in
