...
## Vorschau im Browser testen

Der Vorschau-Server darf `~/Documents` nicht lesen. Daher vor dem Testen kopieren:

```bash
cp -R ~/Documents/Claude/rezepte/. /private/tmp/rezepte-preview/
```

Danach die Konfiguration `rezepte` aus `.claude/launch.json` starten
(Ruby WEBrick, Port 8124). Nach **jeder** Änderung erneut kopieren.

Zwei Fallen dabei:

- Der Browser hält `app.js` im Cache. Kommt eine Änderung nicht an, mit anderer
  Adresse laden: `http://localhost:8124/index.html?v=2`
